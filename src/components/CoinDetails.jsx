...
function CoinDetails() {
  ...
  const btns = ["24h", "7d", "14d", "30d", "60d", "150d", "200d", "1y"]
  ...
  useEffect(() => {
    const fetchCoin = async () => {
      try {
        const { data } = await axios.get(`${server}/coins/${params.id}?x_cg_demo_api_key=${apiKey}`)
        const { data: chartData } = await axios.get(`${server}/coins/${params.id}/market_chart?vs_currency=${currency}&days=${days}&x_cg_demo_api_key=${apiKey}`)
        setCoin(data)
